...
def scrape_bookconnections(path):

    start = time.time()
    print('-'*75)
    print('Scraping bookconnections.org ...')
    print('-'*75)
    ...
    if path == '':
        name = 'bookconnections_data.xlsx'
        # getting the books under each category
        links = []
        nbooks, npages = 0, 0
        homepage = 'https://www.bookconnections.org/tb.cgi?go=1&adv=title&keywordType=all&bookResource=guide#'
        driver.get(homepage)
        while True:           
            # scraping books urls
            ...
            for title in titles:
                try:
                    ...
                    print('The below error occurred during the scraping from bookconnections.com, retrying ..')
                    print('-'*50)
                    print(err)
                    continue
        ...
        with open('bookconnections_links.csv', 'w', newline='\n', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    if path != '':
        ...
        df_links = pd.read_csv('bookconnections_links.csv')
    ...
    print('-'*75)
    print(f'bookconnections.com scraping process completed successfully! Elapsed time {elapsed} mins')
    print('-'*75)
    driver.quit()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        path = sys.argv[1]
    data = scrape_bookconnections(path)
